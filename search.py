...
for i in os.listdir("json"):
   with open(fr"json/{i}","r",encoding="utf-8") as fl:
       ...
       if (namesrch.lower() in current["name"].lower() or namesrch=='') and (set(tagsrch) <= set(current["tags"]) or tagsrch==[]) and (current["name"]!="alltags") and not(set(extagsrch) <= set(current['tags'])):
